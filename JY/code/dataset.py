...
class RembgAugmentation:
    ...
    def __call__(self, image):
        image_before_remove = image.copy()
...
class GenderLabels(int, Enum):
    ...
    @classmethod
    def from_str_incorrect(cls, value: str) -> int:
        """문자열로부터 해당하는 성별 라벨을 찾아 반환하는 클래스 메서드"""
        value = value.lower()
        if value == "male":
            return cls.FEMALE
...
class MaskLabelDataset(Dataset):
    ...
    _file_normals = {
        ...
        "mask4": MaskLabels.MASK,
        "mask5": MaskLabels.MASK,
        "incorrect_mask": MaskLabels.NORMAL,
        "normal": MaskLabels.INCORRECT,
    }
    ...
    _file_masks = {
        ...
        "mask4": MaskLabels.MASK,
        "mask5": MaskLabels.MASK,
        "incorrect_mask": MaskLabels.MASK,
        "normal": MaskLabels.INCORRECT,
    }
    ...
    def __init__(
        ...
        self.transform = None
        self.relabel_image = {
            'incorrect_to_from_normal': ['000020', '004818', '005227'],
    ...
    def setup(self):
        ...
        for profile in profiles:
            ...
            for file_name in os.listdir(img_folder):
                ...
                if _file_name in self.relabel_image['incorrect_to_from_normal']:
                    mask_label = self._file_normals[_file_name]
                elif _file_name in self.relabel_image['incorrect_to_mask']:
                    mask_label = self._file_masks[_file_name]
                else:
                    mask_label = self._file_names[_file_name]

                id, gender, race, age = profile.split("_")

                if id in self.relabel_image['incorrect_gender']:
                    gender_label = GenderLabels.from_str_incorrect(gender)
                else:   
                    gender_label = GenderLabels.from_str(gender)
...
class ProfileDataset(MaskLabelDataset):
    """

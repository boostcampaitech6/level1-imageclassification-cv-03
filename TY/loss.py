...
class MultiLabel(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.mask_loss = criterion_entrypoint("focal")
        self.gender_loss = criterion_entrypoint("cross_entropy")
        self.age_loss = criterion_entrypoint("focal")
        self.multi_label_loss = criterion_entrypoint("focal")

    def forward(self, preds, labels):
        mask_loss = self.mask_loss(preds[0], labels[0])
        gender_loss = self.gender_loss(preds[1], labels[1])
        age_loss = self.age_loss(preds[2], labels[2])
        multilabel_loss = self.multi_label_loss(preds[3], labels[3])
        return [mask_loss, gender_loss, age_loss, multilabel_loss]

...
class BaseAugmentation_for_prefetch:
...
def fast_collate(batch):
    ...
    return tensor, mask_targets, gender_targets, age_targets
    ...
    def __iter__(self):
        ...
        for next_input, next_mask_target , next_gender_target , next_age_target in self.loader:
            with torch.cuda.stream(stream):
                ...
                next_mask_target = next_mask_target.cuda(non_blocking=True)
                next_gender_target = next_gender_target.cuda(non_blocking=True)
                next_age_target = next_age_target.cuda(non_blocking=True)
                next_input = next_input.float().sub_(self.mean).div_(self.std)

            if not first:
                yield input, mask_target, gender_target, age_target
            else:
                first = False

            torch.cuda.current_stream().wait_stream(stream)
            input = next_input
            mask_target = next_mask_target
            gender_target = next_gender_target
            age_target = next_age_target
        yield input, mask_target, gender_target, age_target
...
class MultiLabelDataset(MaskBaseDataset):
    def __init__(self, data_dir, mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246), val_ratio=0.2):
        super().__init__(data_dir, mean, std, val_ratio)
        self.setup()
    
    def gender_transform(self, value: str) -> int:
        """문자열로부터 해당하는 성별 라벨을 찾아 반환하는 클래스 메서드"""
        value = value.lower()
        if value == "male":
            return 0
    ...
    def setup(self):
        ...
        for profile in profiles:
            ...
            for file_name in os.listdir(img_folder):
                ...
                id, gender, race, age = profile.split("_")
                gender_label = self.gender_transform(gender)
                age_label = AgeLabels.from_number(age)
